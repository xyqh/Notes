...
> explicit关键字在C++中只能用于修饰类的构造函数，它用来修饰只有一个参数的类构造函数，以表明该构造函数是显式的，这样可以禁止类对象之间的隐式转换、禁止隐式调用拷贝构造函数。
...
> 显式调用，如B::a和C::a之类的。
>
> 注意：虚继承仅适用于菱形继承的情况，即二义性是由A产生的。假如存在这样的继承关系：`C : A, B`，而AB中有相同命名的成员，则`C : virtual A, B`将无法解决二义性问题，需要显式调用。
>
...
> #### 引用计数
>
> > 引用计数本质上是一个指针，所有的对象通过指针访问同一块空间，实现共享。
> >
> > 即，引用计数是一个临界资源，所以在多线程中，我们必须要保证临界资源访问的安全性，因此在shared_ptr底层中在对引用计数进行访问之前，首先对其加锁，当访问完毕之后，再对其进行解锁。
> >
...
> ```
>
> 线程A执行`p1=p2`，**会先改变ptr的指向，然后才修改引用计数**。由于是多线程，在A执行完步骤一时，还没来得及执行步骤二。这时轮到线程B执行。
>
> 线程B执行`p2=p3`，并且没有被打断，完成了两个步骤。
>
> 此时p2的引用计数已经为0，且原本p2的ptr已经释放，即p1的ptr目前是一个悬空指针。
>
> 接下来线程A继续执行步骤二，此时p1的cnt指向p2的新的cnt。
>
> [C++ shared_ptr的线程安全问题_引用计数是线程安全的,但访问管理的资源不是线程安全的-CSDN博客](https://blog.csdn.net/weixin_43850474/article/details/106328641)
...
### std::move怎么实现资源所有权的转移

> std::move用于将其参数显式地转换为右值引用（rvalue reference）。这告诉编译器可以安全地移动资源，而不是复制。
